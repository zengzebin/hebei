...
import com.ssxt.common.info.BeanComparer;
...
public class DataUtil {
	...
	public static String getStringAfter(String src, String flag, boolean lastFlag) {
		int start = lastFlag ? src.lastIndexOf(flag) : src.indexOf(flag);
		if (start != -1) {
			src = src.substring(start + flag.length(), src.length());
		}
		return src;
	}
	...
	public static Object getParameterInt(String s) {
		if (s == null)
			return null;
		if (s.equals("-1"))
			return null;
	 ...
	 * 
	 * public static String valueOf(long i){ return String.valueOf(i); }
	 */
	...
	/**
	 * 迭代data，返回首个不为空的clz类型对象 如果每个对象为空，则返回null
	 * 
	 * @param clz
	 * @param data
	 * @return T
	 ...
	 *                1.0.0
	 */
	public static <T> T notNull(Class<T> clz, Object... data) {
		T t = null;
		if (data != null)
			for (Object o : data) {
				if (isNull(o)) {
	 ...
	 *                1.0.0
	 */
	public static boolean isAllNull(Object... data) {
		return DataUtil.isNull(DataUtil.notNull(Object.class, data));
	}

	/**
	 * 判断是否全部对象为空 只要有一个不为空，则立即返回
	 * 
	 * @param o
	 * @param defValue
	 * @return
	...
	/**
	 * 判断是否为空，list无元素也返回空判断，以下几个方法雷同 <br/>
	 * 
	 * @param list
	 * @return boolean
	 * @exception @since
	...
	// * @since 1.0.0
	// */
	// public static String getChangeStr(String field,String newLine,String
	// from,String to) {
	...
	public static String judgeChangeStr(Object field, Object newLine, Object from, Object to) {
		...
		try {
			...
			if (to instanceof Double || to instanceof Float) {
				if (0 == parseDouble(to.toString(), 0d))
					to = 0d;
				if (0 == parseDouble(from.toString(), 0d))
					from = 0d;
			...
			// System.out.println((changeFlag?"+":"-")+" "+ field+" "+fromStr
			// +":"+ toStr);
		} catch (Exception e) {
	...
	public static <T> String getAddHistory(String schoolId, Integer userId, String userName, String reason, T domain) {
		String newLine = ConstParam.SYS_NEW_LINE;
		Date date = new Date();
		String history = formatDate(date) + " == TenantID: " + schoolId + " 用户:[" + userName + "] (id:" + userId
				+ ") 因为 " + reason + ", 新增记录" + newLine + "内容:" + JsonHelper.toJSONString(domain);
		return history;
	}
	 ...
	 * @return String
	 */
	public static <T> String getUpdateHistory(String schoolId, Integer userId, String userName, String reason,
			T oldBean, T newBean) {
		String newLine = ConstParam.SYS_NEW_LINE;
		// if(oldBean.getHistory()==null)
		// oldBean.setHistory("");
		Date date = new Date();
		// String history=oldBean.getHistory().trim();
		String oldhistory = "";
		StringBuffer sb = new StringBuffer(oldhistory + newLine + formatTime(date) + " == TenantID: " + schoolId
				+ " 用户:[" + userName + "] (id:" + userId + ") 因为  " + reason + " 修改内容:『" + newLine);

		String format = "  + %s(%s) :【%s】 ==>【%s】\r\n";
		BeanComparer<T> bc = new BeanComparer<T>(oldBean, newBean);
		sb.append(bc.compareFileldChange(format, true));
		// bc.compareFileldChange(format);
		// sb.append(JsonHelper.toJSONString(bc.getResult()));

		sb.append(SpringBeanUtil.compareFileldStr(oldBean, newBean, format));

		String history = sb.append("』").toString();

		// oldBean.setHistory(history);
		return history;
	}
	 ...
	 * @return String
	 */
	public static <T> String getUpdateHistoryByHbm(String schoolId, Integer userId, String userName, String reason,
			T oldBean, T newBean) {
		String newLine = ConstParam.SYS_NEW_LINE;
		// if(oldBean.getHistory()==null)
		// oldBean.setHistory("");
	...
	/**
	 * 
	 * isChange 判断是否变更，以下几个方法雷同，只是参数类型不一样
	 * 
	 * @param from
	 * @param to
	 * @return boolean
	...
	public static boolean responseResult(HttpServletResponse response, String result, String charset) {
		...
		try {
			/*
			 * 有时候会浏览器报错[Break on this error] (<pre>...</pre>) 加上这行就不会
			 */
			// 1.设定返回类型
	...
	/**
	 * 转化成下拉列表书所需的map convert2KVOptionList(作用)<br/>
	 * (适用条件):<br/>
	 * (执行流程):<br/>
	 * (使用方法):<br/>
	 * (注意事项):<br/>
	 ...
	 * @return boolean
	 */
	public static boolean eqIgnoreCase(String obj1, Object obj2) {
		boolean flag = false;
		if (obj1 == null) {
			if (obj2 == null) {
				flag = true;
	...
	public static boolean isBaseDataType(Class clazz) throws Exception {
		return (clazz.equals(String.class) || clazz.equals(Integer.class) || clazz.equals(Byte.class)
				...
				|| clazz.equals(java.sql.Timestamp.class) || clazz.equals(java.sql.Date.class) ||
